...
**Part3 VAD replay 已完成（行动项 #4，见 §6.5）：** 生产 `VadProcessor` 把 Part3 切成 **7 个 1.7–4.0s 短段**，连原本整段失败的 **medium-q8 都全段正确转写，拼接 CER = 1.41%**（turbo-q5 = 0%）。**Part3 在生产管线下不存在 outlier**，「排除 Part3 后 8.23%」是保守说法。

**建议：** 推进 whisper.cpp 落地，首选 **`large-v3-turbo-q5_0`**（延迟最低、Part3 解决、纯语音段精度最好），`medium-q5_0` 作为备选。VAD replay 已消除唯一悬念，可提 Phase 1 第二步关闭、起草落地 ADR。
...
## 6.5 Part3 production VAD replay（ADR-003 必补验证 / 行动项 #4）

**方法。** 直接 `require` 编译好的**生产** `VadProcessor`（`dist-electron/.../vadProcessor.js`，能量式：30ms 窗 / RMS 阈值 0.008 / 300ms hangover / 15s 强制 flush），并复现 `LocalWhisperSTT.processChunk` 的 final 切段路径（30ms 喂入 → `push()` 吐段；开放段 >14s → `softCommit()`；结束 `flush()`）。输入 Part3 原始 16kHz PCM（`test/audio-raw/Part3-Speech-q1.f32`，27.264s）。每段独立用 whisper-cli（CUDA）转写，拼接后按同口径算 CER。

**切段结果。** VAD 把 27.264s 切成 **7 个 segment**，每段 1.71–4.02s，**全部由 300ms 静音 hangover 闭合**，无一触及 15s/14s 阈值。实际语音约 17.1s，其余为句间长沉默——正是这些 ≥4s 内部沉默在「整段单窗」模式下触发了 Whisper 的 `compression_ratio`/`no_speech` fallback。

| segment | 时长 | 闭合原因 |
|---|--:|---|
| seg-01 | 2.64s | hangover |
| seg-02 | 2.16s | hangover |
| seg-03 | 2.52s | hangover |
| seg-04 | 1.86s | hangover |
| seg-05 | 1.71s | hangover |
| seg-06 | 2.19s | hangover |
| seg-07 | 4.02s | hangover |

**逐段转写 + 拼接 CER：**

| 模型 (cuda) | 拼接 CER | 备注 |
|---|--:|---|
| large-v3-turbo-q5 | **0%** | 7 段全对，标点完整 |
| medium-q5 | **1.41%** | 唯一差异「3つ」→「三つ」（1 字） |
| **medium-q8** | **1.41%** | 同上。**整段失败（88.73%）的模型，切段后完全正常** |

**结论。** Part3 的 88.73% outlier **是「27s 含长内部沉默作为单个 Whisper 窗」的产物，不是模型/精度问题**。在生产 VAD 管线下被切成 7 个短段后，**连 medium-q8 都降到 1.41%**。因此：

- 「排除 Part3 后 8.23%」是**保守**说法 —— 生产环境根本没有需要排除的 outlier。
- Blocker #2 关闭：Part3 在 production VAD 下表现优秀，§4.2 的 4 样本 CER 均值在真实管线下只会更低。

> 原始数据：`spike/whisper-cpp/vad-replay/segments.json`、`vad-replay-results.json`。
...
| Part3 outlier | 经 VAD replay 解释或被切段消除 | VAD 切 7 段后 q8 也仅 1.41%、turbo 0% | ✅ 已终验（§6.5） |
...
2. ~~补做 Part3 production VAD replay（ADR-003 行动项 #4）~~ **已完成（§6.5）**：VAD 切 7 段，q8 也仅 1.41% CER，outlier 消除。Part4 尾部幻觉的 VAD 验证可在落地实现时随集成测试一并覆盖。
